...
from financepy.products.rates import FinFixedLeg, FinFloatLeg
...
from financepy.finutils.FinCalendar import FinCalendarTypes,  FinDateGenRuleTypes
from financepy.finutils.FinCalendar import FinCalendar, FinBusDayAdjustTypes
from financepy.finutils.FinSchedule import FinSchedule
from financepy.finutils.FinHelperFunctions import labelToString, checkArgumentTypes
from financepy.finutils.FinGlobalTypes import FinSwapTypes
from financepy.market.curves.FinDiscountCurve import FinDiscountCurve


class ExFinFixedLeg(FinFixedLeg):

    def get_payments(self):
        data = {
            "PAY_DATE": [str(date) for date in self._paymentDates],
            "ACCR_START": [str(date) for date in self._startAccruedDates],
            "ACCR_END": [str(date) for date in self._endAccruedDates],
            "DAYS": self._accruedDays,
            "YEARFRAC": self._yearFracs,
            "RATE": self._rates,
...
class ExFinFloatLeg(FinFloatLeg):
    ...
    def get_valuation(self):
        ...
        data = {
            ...
            "DAYS": self._accruedDays,
            "YEARFRAC": self._yearFracs,
            "IBOR": [rate * 100 for rate in self._rates],
            "PAYMENT": self._payments,
            "DF": self._paymentDfs,
            "PV": self._paymentPVs,
            "CUM PV": self._cumulativePVs,
        }
        df = pd.DataFrame(data=data)
        return df


class ExFinFloatLeg2(object):
