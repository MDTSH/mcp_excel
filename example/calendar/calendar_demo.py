...
# -*- coding: utf-8 -*-
"""
MCP Calendar Function Demo Program

This program demonstrates various uses of MCP calendar functions, including:
- Business day adjustment rules
- Value date calculation
- Expiry date calculation
- Tenor calculation
- Day count calculation, etc.
...
# Create McpCalendar object based on specified currency and holiday data
...
# Create McpCalendar object based on specified currency pair and holiday data
def McpNCalendar(ccys, holidays):
    cal = McpCalendar(ccys, holidays)
    return cal

"""Case 1) Business Day Rules"""
...
def test_adjust():
    ...
    print(f"Following adjustment for {date}: {adjust_following_date}")
    
    # Modified Following: Roll to next business day, but if it crosses to next month, roll back to previous business day
    adjust_modified_following_date = cal.Adjust(date, DateAdjusterRule.ModifiedFollowing)
    print(f"Modified Following adjustment for {date}: {adjust_modified_following_date}")
    
    # Preceding: Roll back to previous business day
    adjust_preceding_date = cal.Adjust(date, DateAdjusterRule.Preceding)
    print(f"Preceding adjustment for {date}: {adjust_preceding_date}")
    
    results = [[adjust_following_date, adjust_modified_following_date, adjust_preceding_date]]
    print(f"Results DataFrame: {results}")
...
def test_valuedate():
    ...
    print(f"ValueDate for {date}: {result}, Expected: {expected_date}")
    assert result == expected_date


"""Case 3) Option Premium Payment Date Rules"""
...
def test_expiredate():
    ...
    print(f"FXOExpiryDate for {date}: {result}, Expected: {expected_date}")
    assert result == expected_date


"""Case 6-1) Add Business Days Example"""
...
def test_add_day():
    ...
    print(f"AddBusinessDays for {date}: {result}, Expected: {expected_date}")
    assert result == expected_date


"""Case 6-2) Add Tenor Example"""
...
def test_add_tenor():
    ...
    print(f"AddPeriod for {date} with tenor '2W': {result}, Expected: {expected_date}")
    assert result == expected_date


"""Case 7) Calculate Expiry Date and Delivery Date"""

def test_expiry_delivery_dates():
    """Test option expiry date and delivery date calculation"""
    print("    Test Input:")
    print("    - Reference Date: 2024-6-26")
    print("    - Tenor: ON (Overnight)")
    print("    - Currency Pair: USD/CNY")
    
    cal = McpNCalendar(['USD', 'CNY'], [usd_dates, cny_dates])
    reference_date = '2024-6-26'
    spot_date = cal.ValueDate(reference_date)
    print(f"    Spot Value Date: {spot_date}")
    
    expire_date = cal.FXOExpiryDateFromTenor(reference_date, 'ON', spot_date)
    print(f"    Calculated Expiry Date: {expire_date}")
    ...
    print(f"    Expected Expiry Date: {expected_expire_date}")
...
"""Case 8) Check if Holiday"""

def test_is_business_day():
    """Test business day checking functionality"""
    print("    Test Dates:")
    date1 = '2009-1-1'  # New Year's Day, should be holiday
    date2 = '2009-1-2'  # Business day
    print(f"    - Date 1: {date1} (New Year's Day)")
    print(f"    - Date 2: {date2} (Regular business day)")
    
    cal = GetCurrencyCalendar('USD', usd_dates)
    result1 = cal.IsBusinessDay(date1, '')
    result2 = cal.IsBusinessDay(date2, '')
    print(f"    {date1} is business day: {result1}")
    print(f"    {date2} is business day: {result2}")
    
    assert result1 == False, f"New Year's Day should be holiday, but returned {result1}"
    assert result2 == True, f"January 2nd should be business day, but returned {result2}"


"""Case 9) Calculate Near and Far Value Dates for Tenor"""
...
def test_valuedate_tenor():
    ...
    print(f"ValueDate for {reference_date} with tenor {tenor}: Near Date: {near_date}, Far Date: {far_date}")
    
    expected_near_date = '2024-06-28'
    expected_far_date = '2024-07-29'
    print(f"Expected Near Date: {expected_near_date}")
    print(f"Expected Far Date: {expected_far_date}")
    
    assert near_date == expected_near_date
    assert far_date == expected_far_date


"""Case 11) Calculate Day Count"""
...
def test_day_counter():
    ...
    print(f"YearFraction from {start_date} to {end_date}: {result}")
    
    expected = 0.519444
    print(f"Expected YearFraction: {expected}")
    assert result == expected


def main():
    test_cases = [
        ("Case 1) Business Day Rules", test_adjust),
        ("Case 2) Value Date Rules", test_valuedate),
        ("Case 3) Option Premium Payment Date Rules", test_valuedate2),
        ("Case 5) Expiry Date Rules", test_expiredate),
        ("Case 6-1) Add Business Days Example", test_add_day),
        ("Case 6-2) Add Tenor Example", test_add_tenor),
        ("Case 7) Calculate Expiry Date and Delivery Date", test_expiry_delivery_dates),
        ("Case 8) Check if Holiday", test_is_business_day),
        ("Case 9) Calculate Near and Far Value Dates for Tenor", test_valuedate_tenor),
        ("Case 11) Calculate Day Count", test_day_counter),
    ]
